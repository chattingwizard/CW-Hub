...
export default function Login() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('60 seconds') || err.message?.includes('few minutes')) {
        setResetCooldown(60);
      }
    }
  };
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                {resetSent && (
                  <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg px-4 py-3 text-emerald-400 text-sm">
                    Check your email for a password reset link. Also check your spam folder.
                  </div>
                )}
                <div>
                ...
                >
                  ...
                  ) : resetCooldown > 0 ? `Resend in ${resetCooldown}s` : resetSent ? 'Resend Reset Link' : 'Send Reset Link'}
                </button>
              </form>
            </>
          ) : (
